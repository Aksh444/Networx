 ...
 let [show,setShow]= useState(false) 
  ...
  return (
    <div className='w-full h-screen bg-white flex flex-col items-center justify-start gap-[10px]'> 
      ...
      <form className='w-[90%] max-w-[350px] h-[550px] md:shadow-xl flex flex-col justify-center gap-[10px] p-[15px]' onSubmit={handleSignIn}>
          ...
          <button className='w-[100%] h-[40px] rounded-full bg-[#00B2A9] mt-[30px] text-white' disabled={loading}>{loading?"loading...":"Sign In"}</button>
          <p className='text-center'>Want to make a new account?<span className='text-[#00B2A9] cursor-pointer' onClick={()=>navigate("/signup")}>Sign Up</span></p>    
      </form>
    </div>
  )
}
